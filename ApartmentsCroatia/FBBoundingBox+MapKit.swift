...
extension FBBoundingBox {
	...
	init(mapRect: MKMapRect) {
		let topLeft = mapRect.origin.coordinate
		let bottomRight = MKMapPoint.init(x: MKMapRect MKMapRectGmapRect.maxXRectGmapRect.maxYte)
	...
	func mapRect() -> MKMapRect {
		let topLeft  = MKMapPoint.init(CLLocationCoordinate2DMake(CLLocationDegrees(x0), CLLocationDegrees(y0)))
		let botRight  = MKMapPoint.init(CLLocationCoordinate2DMake(CLLocationDegrees(xf), CLLocationDegrees(yf)))
